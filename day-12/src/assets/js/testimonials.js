...
const testimonial = new Promise((resolve, reject) => {
  ...
  xhr.open("GET", "https://api.npoint.io/4ea849dc83431ea43456", true);
  // Get -> Methode
  // yg-2 -> Alamat api
    ...
    response.forEach((item) => {
      ...
      <div class="col-md-6 col-lg-4 mb-4">
        <div class="card shadow-sm">
            <img src="${item.image}"
            alt="testimonial" class="card-img-top img-fluid card-body bg-light">
            <div class="card-body bg-light">
              <p class="card-text text-color">${item.content}</p>
              <p class="card-text text-end text-color font-weight-bold">- ${item.author}</p>
              <p class="card-text text-end text-color fw-bold">${item.rating} <i class="fa-solid fa-star text-warning"></i></p>
          </div>
        </div>
      </div>`;
    });
      ...
      dataFilter.forEach((item) => {
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body bg-light">
                <p class="card-text text-color">${item.content}</p>
                <p class="card-text text-end text-color font-weight-bold">- ${item.author}</p>
                <p class="card-text text-color text-end font-weight-bold">${item.rating} <i class="fa-solid fa-star text-warning"></i></p>
            </div>
        </div>
    </div>`;
      });
